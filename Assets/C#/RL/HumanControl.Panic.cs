...
public partial class HumanControl : MonoBehaviour
{
    ...
    void UpdatePanicLevel()
    {
        ...
        _myNavMeshAgent.speed = currentSpeed;
        ...
        float distanceTerm = Mathf.Clamp01(exitDistance / 20); //距离出口的距离
        // 综合计算
        panicLevel = Mathf.Clamp01(
           0.4f* healthTerm+
            0.6f * distanceTerm
            );
        ...
        currentSpeed = Mathf.Lerp(startdesiredSpeed, MaxSpeed, panicLevel); 
    ...
    private void UpdateBehaviorModel()
    {
        ...
        if (UsePanic)
        {
            ...
            else if (panicLevel > 0.6 && panicLevel <= 1) { CurrentState = 2; }
    ...
    private float _panicDelayOffset;       //用来实现异步移动
    private void UpdatePanicDestination()
    {
        ...
        // 动态调整参数
        _myNavMeshAgent.speed = Mathf.Lerp(5f, 10f, panicLevel);
        _myNavMeshAgent.angularSpeed = Mathf.Lerp(120f, 360f, panicLevel);
    }
    ...
    private void AddMovementNoise()
    {
        ...
        if (Random.value < 0.3f)
        {
            ...
            _myNavMeshAgent.velocity += noise;
        }
    }
   ...
   private Vector3 GetFallbackPosition()
    ...
    private void AddMovementNoise(float intensity = 0.2f)
    {
        Vector3 noise = new Vector3(
